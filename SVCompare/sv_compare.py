...
def compare_casts(cast_ref, cast_comp, ref_color_str = def_ref_color_str,
    comp_color_str = def_comp_color_str, save_plot = False, **kwargs):
    ...
    #print diffs
    results = '=====================================\n'
    results += ' Sound Speed Cast Comparison Results\n'
    results += '=====================================\n'
    results += '%i Values Compared' % len(diffs) + '\n'
    results += 'Max Absolute Difference: %0.3f' % np.max(np.abs(diffs)) + '\n'
    results += 'Average Difference: %0.3f' % np.mean(diffs) +'\n'
    results += 'Average Absolute Difference: %0.3f' % np.mean(np.abs(diffs)) + '\n'
    results += 'StDev Differences: %0.3f' % np.std(diffs)
    ...
    if save_plot:
        plt.savefig(kwargs['savepath'], dpi=300)
    plt.show()

    return results

def save_name(ref_color, comp_color, ext):
    return 'Results_' + ref_color + '_' + comp_color + '.' + ext
...
def velocipy_compare(prof1, prof2):
...
def main():
    if len(sys.argv) == 4:
        ...
        # Run the comparisons
        print compare_casts(get_data_only(prof1), get_data_only(prof2))
        print velocipy_compare(prof1, prof2)
